      ...
      <p><a href="#">Link</a></p> -->
			...
			<div class="col-sm-8 text-left">
				...
				<form action="${linkEntradaServlet }" method="post" name="dados" onSubmit="return enviarDadosAlteraUsuario();">
					...
					<input type="password" name="senha" value="${usuario.senha}" required="required" class="dados-cadastro">
					<br>
					...
					<input type="text" name="data" class="dados-cadastro" placeholder="DD/MM/YYYY" pattern="\d{2}/\d{2}/\d{4}" 
					value="<fmt:formatDate value="${usuario.dataNascimento}" pattern="dd/MM/yyyy" />" required="required" ></input>
				...
				<ul>
						...
						<c:if test="${cartoes!=null}">
							...
							<c:forEach items="${cartoes}" var="cartao">
								<c:if test="${usuario.id ==  cartao.idUser}">
									<li class="card-slot">Nome do titular: ${cartao.titular} <br>Numero do
										cartao: ${cartao.numeroCartao}<br> Data de Vencimento:
										<fmt:formatDate value="${cartao.dataVencimento}" pattern="dd/MM/yyyy" /><br>
										 <a href="entrada?acao=MostraCartao&id=${cartao.idUser}&numeroCartao=${cartao.numeroCartao}"><button type="button">Editar Cartão</button></a> 
										 <a href="entrada?acao=RemoveCartao&numero=${cartao.numeroCartao}"><button type="button">Remover Cartão</button></a>
									</li>
									<br>
								</c:if>
							</c:forEach>
					...
					<c:if test="${usuarioLogado.isAdm =='FALSE'}">
						<li class="lista-cartao"><a href="entrada?acao=FormNovoCartao&id=${usuario.id}"><button type="button" class="botao-novo">Cadastrar Novo Cartão</button></a></li>
					</c:if>
				</ul>
				<br> <br> <br> <br>
